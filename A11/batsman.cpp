...
int main()
{
	...
	strcpy(usfd_addr.sun_path, "balltobat");  
    ...
    cout<<"Batsman recieved the ball from bowler!"<<endl;
    
    string data = ReadFile(fd);
    int run;
    int start = 0;
    int end = data.find(" ");
    ...
    cout<<"Batsman sent ball to empire!"<<endl;

    cout<<"Notifying EMpire about runs!";
    send(nusfd, s.c_str(),s.size(),0);
}